...
function addFailWithBody(curl_text) {
    if (curl_text.includes('--fail-with-body'))
        return curl_text;
    if (/^curl\b/i.test(curl_text.trim()))
        return `${curl_text} --fail-with-body`;
    return `curl ${curl_text} --fail-with-body`;
}
function ensureHttpsIfMissing(curl_text) {
    if (curl_text.includes('http://') || curl_text.includes('https://'))
        return curl_text;
    return curl_text.replace(/\b(curl\s+)(\S+\.\S+)/i, '$1https://$2');
}
function maybeAddInsecureForDebug(curl_text) {
    if (curl_text.includes(' -k') || curl_text.includes(' --insecure'))
        return curl_text;
    if (/^curl\b/i.test(curl_text.trim()))
        return `${curl_text} -k`;
    return `curl ${curl_text} -k`;
}
function parseHttpStatusFromVerbose(stderr_text) {
...
function pickEvidence(stderr_text, needle) {
